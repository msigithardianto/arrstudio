<?php
require_once __DIR__ . '/LuaGen.php';
...
class FullGen {
    public static function generateFullScript($nodes, $W, $H) {
        ...
        foreach ($sorted as $n) {
            ...
            $L[] = "{$v}.BackgroundColor3 = Color3.new(" . LuaGen::c3($n['bg']) . ')';
            ...
            if (!empty($n['gradient'])) {
                ...
                $L[] = "    _g.Color = " . LuaGen::colorSeq($n['gradient']['keypoints']);
                $L[] = "    _g.Transparency = " . LuaGen::numSeq($n['gradient']['keypoints']);
                $L[] = "    _g.Parent = {$v}";
                $L[] = 'end';
            }
            ...
            if (in_array($n['robloxClass'], ['TextLabel','TextButton','TextBox'])) {
                ...
                $L[] = "{$v}.Text = " . LuaGen::q($txt);
                $L[] = "{$v}.TextColor3 = Color3.new(" . LuaGen::c3($n['fg']) . ')';
                ...
                if ($n['robloxClass'] === 'TextBox') {
                    $L[] = "{$v}.PlaceholderText = " . LuaGen::q($n['placeholder'] ?: '');
                }
            ...
            if ($n['robloxClass'] === 'ImageLabel') {
                $L[] = "{$v}.Image = " . LuaGen::q($n['src']);
            }

            $L[] = "{$v}.BorderSizePixel = " . ($n['borderW'] > 0 ? max(1, (int)$n['borderW']) : 0);
            if ($n['borderW'] > 0) {
                $L[] = "{$v}.BorderColor3 = Color3.new(" . LuaGen::c3($n['borderColor']) . ')';
            }
    ...
    private static function generateInlineBehavior($nodes, $vmap, $W = 800, $H = 600) {
        $L = [];
        ['pairs' => $pairs, 'buttons' => $buttons, 'usedBtn' => $usedBtn] = LuaGen::detectPairs($nodes);
        ...
        foreach ($pairByBtn as $group) {
            ...
            foreach ($group['panels'] as $panel) {
                ...
                $ease = LuaGen::mapEasing($panel['transition']['easing'] ?? 'ease-out');
        ...
        foreach ($nodes as $n) {
            ...
            $ease = LuaGen::mapEasing($n['transition']['easing']);
            $baseX = $n['x'];
            $baseY = $n['y'];
